  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        setTimeout(() => setDialogOpen(false), 4000); // Auto close after 4s
      } else {
  ...
  return (
    <section className="max-w-2xl mx-auto space-y-6 pt-28 pb-16 px-6 bg-gray-50">
      ...
      >
        ...
        <button
          type="submit"
          className="bg-yellow-400 hover:bg-yellow-500 text-black py-3 px-5 rounded font-semibold w-full transition-all"
        >
          Send Message
        </button>
      </form>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="text-center space-y-4">
          ...
          <DialogHeader>
            <DialogTitle className="text-green-600 text-center font-bold text-xl">
